...
public enum FarmDefinition implements StringRepresentable {
	...
	protected IFarmPropertiesBuilder wateredProperties(IFarmPropertiesBuilder builder) {
		return builder.setWaterConsumption((hydrationModifier) -> (int) (20 * hydrationModifier))
				.setFertilizer(5);
	}
	...
	public void registerCircuits(ICircuitRegistration circuits) {
		circuits.registerCircuit(ForestryCircuitLayouts.MANAGED_FARM, CoreItems.ELECTRON_TUBES.stack(tube, 1), managed);
		circuits.registerCircuit(ForestryCircuitLayouts.MANUAL_FARM, CoreItems.ELECTRON_TUBES.stack(tube, 1), manual);
	}
